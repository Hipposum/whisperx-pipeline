...
```
/PachcaVideos/          <- папка с видео для транскрибации (videos_folder)
/settings/
    prompts/            <- промпты (prompts_folder)
        timeline_system.txt
        timeline_user.txt
        report_system.txt
        report_user.txt
...
Промпты хранятся в двух местах (используется Яндекс Диск если доступен, иначе локальные):

### Вариант 1: Локально (в репозитории)

Отредактируйте файлы в `prompts/` и запушьте на GitHub. При следующем `git clone` Kaggle
получит обновлённые промпты.

### Вариант 2: Яндекс Диск (без пересборки)

Перейдите в Яндекс Диск → `/settings/prompts/` и отредактируйте файлы прямо там.
При следующем запуске пайплайн подтянет актуальные версии.
